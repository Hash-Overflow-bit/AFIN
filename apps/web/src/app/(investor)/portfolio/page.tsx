  ...
  return (
    ...
    >
      <div>
        ...
        <p className="text-[#79628c] dark:text-on-dark-muted mt-2 text-sm">Overview of your investments, cashflows, and holdings.</p>
      </div>
      ...
      {(user?.investorProfile?.kycStatus === 'DOCUMENTS_SUBMITTED' || user?.kycStatus === 'DOCUMENTS_SUBMITTED') && (
        <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 p-4 rounded-xl flex items-center gap-3">
          ...
          <div>
            <h3 className="text-amber-800 dark:text-amber-300 font-bold text-sm">Your documents are under review</h3>
            <p className="text-amber-700 dark:text-amber-400/80 text-xs mt-0.5">Administrators are currently reviewing your submitted KYC documents. Full platform features will unlock upon approval.</p>
          </div>
        </div>
      )}

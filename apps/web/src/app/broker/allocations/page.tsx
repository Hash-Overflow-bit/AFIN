...
export default function BrokerAllocationsPage() {
  ...
  const handleRunAllocation = async () => {
    if (!preview) return;
    if (!confirm(`Are you sure you want to run allocation for ${preview.bondName}? This action cannot be undone.`)) return;
  ...
  return (
    <div className="bg-surface-canvas-light text-ink min-h-full">
      <div className="mb-8">
        <h1 className="text-[30px] font-medium leading-[1.2] text-primary tracking-tight">Allocation Manager</h1>
        <p className="text-[16px] text-ink/70 mt-1">Preview and execute bond distributions to verified investors.</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-surface-canvas-light p-4 rounded-lg border border-hairline-cloud shadow-sm text-center">
              <p className="text-[12px] uppercase font-bold text-ink/60 tracking-console mb-1">Total Supply (MZN)</p>
              <p className="text-[24px] font-medium text-primary">{preview.totalSupply.toLocaleString()}</p>
            </div>
            <div className="bg-surface-canvas-light p-4 rounded-lg border border-hairline-cloud shadow-sm text-center">
              <p className="text-[12px] uppercase font-bold text-ink/60 tracking-console mb-1">Total Demand (MZN)</p>
              <p className="text-[24px] font-medium text-primary">{preview.totalDemand.toLocaleString()}</p>
            </div>
            <div className="bg-surface-canvas-light p-4 rounded-lg border border-hairline-cloud shadow-sm text-center">
              <p className="text-[12px] uppercase font-bold text-ink/60 tracking-console mb-1">Allocation Ratio</p>
              <p className="text-[24px] font-medium text-primary">{(preview.ratio * 100).toFixed(2)}%</p>
            </div>
            <div className="bg-surface-canvas-light p-4 rounded-lg border border-hairline-cloud shadow-sm text-center">
              <p className="text-[12px] uppercase font-bold text-ink/60 tracking-console mb-1">Projected Distribution</p>
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              {running ? 'Executing...' : 'Execute Allocation'}
            </button>
          </div>
          ...
          <div className="bg-surface-canvas-light rounded-lg border border-hairline-cloud overflow-hidden shadow-level-1">
            <div className="p-4 bg-surface-press-light border-b border-hairline-cloud flex items-center gap-2">
              ...
              <h2 className="font-bold text-ink">Projected Investor Allocations</h2>
            </div>
            <table className="w-full text-left text-[14px] text-ink">
              <thead className="bg-surface-canvas border-b border-hairline-cloud text-[12px] uppercase font-bold text-ink/60 tracking-console">
                <tr>
                  ...
                  <th className="px-6 py-4 text-right">Requested (MZN)</th>
                  <th className="px-6 py-4 text-right">Allocated (MZN)</th>
                ...
                {preview.projectedAllocations.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="px-6 py-12 text-center text-ink/50">
                      ...
                      No verified payments found for this bond.
                    </td>
                  </tr>
                ) : (

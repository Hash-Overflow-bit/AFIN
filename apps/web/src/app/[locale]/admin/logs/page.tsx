      ...
      <div className="bg-white rounded-xl border border-hairline-cool shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-24 text-[#79628c] space-y-2">
            ...
            <p className="text-lg font-semibold">{t('noLogsFound')}</p>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface-night/5 text-xs font-semibold text-[#79628c] border-b border-hairline-cool">
                  <th className="p-4">{t('colTimestamp')}</th>
                  <th className="p-4">{t('colAction')}</th>
                  <th className="p-4">{t('colActor')}</th>
                  <th className="p-4">{t('colResourceType')}</th>
      ...
      {selectedLog && (
        <div className="fixed inset-y-0 right-0 max-w-lg w-full bg-white z-50 shadow-2xl flex flex-col border-l border-hairline-cool animation-slide-in">
          <div className="p-6 border-b border-hairline-cool flex items-center justify-between bg-slate-50">
            <div>
              <h3 className="font-bold text-[#1f1633] text-lg">{t('drawerTitle')}</h3>
              <p className="text-xs text-[#79628c] mt-0.5">{t('drawerSubtitle', { id: selectedLog.id })}</p>
            </div>
            <button 
          ...
          <div className="flex-1 p-6 overflow-y-auto space-y-6">
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm border-b border-hairline-cool pb-4">
                <div>
                  <span className="block text-xs font-semibold text-[#79628c] uppercase">{t('lblActionName')}</span>
                  <span className="font-mono font-bold text-red-700 mt-1 inline-block">{selectedLog.action}</span>
                </div>
                <div>
                  <span className="block text-xs font-semibold text-[#79628c] uppercase">{t('lblTimestamp')}</span>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm border-b border-hairline-cool pb-4">
                <div>
                  <span className="block text-xs font-semibold text-[#79628c] uppercase">{t('lblActorEmail')}</span>
                  <span className="font-semibold text-ink-deep mt-1 inline-block">{selectedLog.user?.email || 'System'}</span>
                </div>
                <div>
                  <span className="block text-xs font-semibold text-[#79628c] uppercase">{t('lblIp')}</span>
              ...
              {selectedLog.resourceType && (
                <div className="text-sm border-b border-hairline-cool pb-4">
                  <span className="block text-xs font-semibold text-[#79628c] uppercase">{t('lblResourceRef')}</span>
            ...
            <div className="space-y-2">
              <span className="block text-xs font-semibold text-[#79628c] uppercase">{t('lblJsonPayload')}</span>

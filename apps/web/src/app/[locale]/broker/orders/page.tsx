...
export default function BrokerOrdersPage() {
  const t = useTranslations("BrokerOrders");
  ...
  const handleApprove = async (id: string) => {
    ...
    try {
      ...
      alert(error.response?.data?.message || t('failedApprove'));
    } finally {
      setActioningId(null);
    }
  };
      ...
      <div className="space-y-8">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-[#e5e7eb] p-12 text-center shadow-sm">
            ...
            <h3 className="text-[20px] font-bold text-[#1f1633] mb-2">{t('inboxZero')}</h3>
            <p className="text-[#79628c]">{t('inboxZeroDesc')}</p>
          </div>
        ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-[#cfcfdb] bg-[#f9fafb]">
                    <th className="py-4 px-6 text-[12px] font-semibold text-[#79628c] uppercase tracking-wider">{t('colOrderNo')}</th>
                    <th className="py-4 px-6 text-[12px] font-semibold text-[#79628c] uppercase tracking-wider">{t('colInvestor')}</th>
                    <th className="py-4 px-6 text-[12px] font-semibold text-[#79628c] uppercase tracking-wider">{t('colBond')}</th>
                  ...
                  {orders.map((order) => (
                    <tr key={order.id} className="hover:bg-[#f9fafb] transition-colors">
                      ...
                      <td className="py-4 px-6 whitespace-nowrap text-right space-x-2">
                        ...
                        {order.status !== 'PENDING_REVIEW' && (
                           <span className="text-[12px] text-[#79628c] italic">{t('txtProcessed')}</span>
                        )}
                      </td>
                    </tr>
                  ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#150f23]/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-xl shadow-[rgba(0,0,0,0.1)_0_10px_15px_-3px] w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-6 border-b border-[#e5e7eb]">
              <h2 className="text-[20px] font-bold text-[#1f1633]">{t('modalRejectTitle')}</h2>
            </div>
            ...
            <form onSubmit={submitReject} className="p-6 space-y-4">
              <div>
                ...
                ></textarea>
                <p className="text-[12px] text-[#79628c] mt-1">{t('modalRejectReasonHint')}</p>
              </div>

...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        typedErr.response?.data?.message || t('invalidCredentials'),
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <main className="flex-1 flex items-center justify-center p-6 z-10 relative">
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-950/20 border border-red-500/30 rounded-md text-red-400 font-mono text-xs shadow-level-1">
            <span className="font-bold uppercase tracking-console">{t('errorPrefix')}</span>{" "}
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          >
            ...
            {isLoading ? t('authenticating') : t('loginToWorkspace')}
          </button>
        </form>

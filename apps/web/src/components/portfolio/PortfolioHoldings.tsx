...
export default function PortfolioHoldings({ holdings }: Props) {
  ...
  return (
    ...
    >
      <h3 className="text-lg font-bold text-[#1f1633] dark:text-white mb-6">{t('activeHoldingsTitle')}</h3>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-[#4b5563]">
          <thead className="text-xs text-[#79628c] dark:text-on-dark-muted uppercase tracking-wider bg-[#f9fafb] dark:bg-[#1a1130] border-b border-[#e5e7eb] dark:border-hairline-violet">
            <tr>
              <th className="px-4 py-3 font-medium rounded-tl-lg">{t('bond')}</th>
              <th className="px-4 py-3 font-medium">{t('faceValue')}</th>
              <th className="px-4 py-3 font-medium">{t('yield')}</th>
              <th className="px-4 py-3 font-medium rounded-tr-lg">{t('status')}</th>
            </tr>
          </thead>
          <tbody>
            {holdings.length === 0 ? (
              ...
              holdings.map((holding, i) => (
                <tr key={holding.id} className="border-b border-[#e5e7eb] dark:border-hairline-violet hover:bg-[#f9fafb] dark:hover:bg-[#1f1735]/40 transition-colors">
                  <td className="px-4 py-4">
                    ...
                    <div className="text-xs text-[#79628c] dark:text-on-dark-muted mt-1">{t('acquired')}: {new Date(holding.acquiredAt).toLocaleDateString()}</div>
                  </td>
